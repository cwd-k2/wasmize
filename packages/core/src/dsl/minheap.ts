...
export interface MinHeapHandle {
  /** Inserts a (priority, value) pair into the heap. */
  insert(priority: ExprInput, value: ExprInput): FuncGen<void>;
  /** Removes the minimum element and stores its priority/value in the given refs. */
  extractMin(dstPri: WasmRef<"i32">, dstVal: WasmRef<"i32">): FuncGen<void>;
  /** Reads the minimum priority without removing it. */
  peekPriority(): ChainableExpr;
  /** Reads the minimum value without removing it. */
...
/**
 * Creates a min-heap (priority queue) backed by linear memory.
 *
 * Memory layout: interleaved `[pri0, val0, pri1, val1, ...]` using i32 pairs.
 * Generator form: `const heap = yield* MinHeap(base)` allocates internal
 * locals and returns a handle with `insert`/`extractMin`/`peek`/`notEmpty`/`reset`.
 *
 * @param base - Base byte offset for the heap's backing i32 array
 */
export function* MinHeap(base: ExprInput): Generator<any, MinHeapHandle, any> {
      ...
      return (function* () {
        ...
        yield* Ctrl.block(function* () {
          yield* Ctrl.loop(function* () {
            ...
            yield* Ctrl.when(arr.load(other.mul(2)).le(arr.load(idx.mul(2))), function* () {
              yield* Ctrl.br(2);
            });
      ...
      return (function* () {
        ...
        yield* Ctrl.block(function* () {
          yield* Ctrl.loop(function* () {
            ...
            // If right child exists and has smaller priority, use right
            yield* Ctrl.when(
              other.add(1).lt(sz).and(arr.load(other.add(1).mul(2)).lt(arr.load(other.mul(2)))),
              () => [other.incrBy(1)],
            );

            // If current priority <= smallest child priority, done
            yield* Ctrl.when(arr.load(idx.mul(2)).le(arr.load(other.mul(2))), function* () {
              yield* Ctrl.br(2);
            });
