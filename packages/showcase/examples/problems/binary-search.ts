 ...
 */
import { locals, Type, Mod, Mem, Ctrl, Loc } from "wasmize/dsl/compiler";
...
export function problem5_binary_search() {
  return compileWithWat<{
    ...
    const binary_search = yield* Mod.func(
      ...
      function* (len, target) {
        const [lo, hi, mid, v] = yield* locals([Type.i32, 0], [Type.i32, len.sub(1)], Type.i32, Type.i32);
    ...
    const search_batch = yield* Mod.func(
      ...
      function* (len, tbase, tcount) {
        const [ti, sum] = yield* locals(Type.i32, [Type.i32, 0]);
