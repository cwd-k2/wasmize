 ...
 *
 * Approach: Triple-nested loop with stride optimization.
 * Memory layout: three n×n i32 matrices at fixed offsets.
 * Complexity: O(n³) time.
 * DSL features: Ctrl.grid (outer i,j loops), Ctrl.range (inner k loop), stride addressing.
 */
...
export function problem8_matmul() {
  return compileWithWat<{ matmul: (n: number) => number }>(function* () {
    ...
    yield* Mod.exportFunc("matmul", { n: Type.i32 }, function* (n) {
      const [k, sum, nn, baseB, baseC, n4, iRowA, jColB, bStride] =
        yield* locals(
      ...
      // Ctrl.grid replaces the outer i,j double loop
      yield* Ctrl.grid([n, n], (i, j) => [
        iRowA.set(i.mul(n4)),
        sum.set(0),
        jColB.set(j.mul(4).add(baseB)),
        bStride.set(0),
        ...
        // C[i][j] = sum
        Mem.store(iRowA.add(j.mul(4)).add(baseC), sum),
      ]);
