...
import { OP } from "wasmize/wasm/opcodes";
...
describe("Wasm MVP Spec Coverage", () => {
  test("coverage report", () => {
    const categories = [...new Set(WASM_MVP_CATALOG.map((i) => i.category))] as Category[];
    ...
    for (const cat of categories) {
      const missing = WASM_MVP_CATALOG.filter((i) => i.category === cat && !opValues.has(i.opcode));
      if (missing.length > 0) {
