...
2. **IR 層** — 36 種の `IRNode` discriminated union + `IR.*` ビルダー API
...
- **メモリ抽象** — `Mem.i32Array`, `Mem.i32Array2D`, `Mem.byteGrid` でストライド計算を隠蔽
- **Struct 型** — `Struct({ x: "i32", y: "f64", r: "u8" })` でメモリレイアウトを型安全に管理。`FieldAccessor` による OOP スタイルアクセス
- **データ構造** — Queue, Stack, RingBuffer, BitSet, MinHeap, HashMap（Generator ファクトリパターン）
- **stdlib** — math（pow, clamp, lerp）, mem（memcpy, memset, memcmp）, sort（quicksort, insertion sort）
...
- **ランタイム** — `instantiate()`, AsyncBridge, WorkerPool, Marshal
- **JS メタプログラミング** — Config 配列展開、ファクトリ関数、`Meta.each/times/when/sum` マクロ
- **ツーリング** — WAT 出力, IR 統計, 最適化レポート, Generator Intercept, Profiling, Bounds Guard
...
packages/
  core/                   # wasmize（npm publish 対象）
    src/
      ...
      wasm/               # IR 定義・Codegen・Module Builder・Encoder・Opcodes・Optimizer
      stdlib/             # 再利用可能 Wasm 関数ライブラリ（math, mem, sort）
      runtime/            # ホスト統合ユーティリティ（instantiate, marshal, worker-pool）
      __tests__/          # ライブラリテスト
